...
class Model(nn.Module):
    def __init__(self):
        ...
        self.config = BertConfig.from_pretrained(args.pretrained_config_path)
        self.bert = BertModel.from_pretrained(args.pretrained_model_path, config=self.config)

    def forward(self, input_ids, attention_mask, encoder_type=args.encoder_type):
        output = self.bert(input_ids, attention_mask, output_hidden_states=True)

        if encoder_type == 'fist-last-avg':
            # 第一层和最后一层的隐层取出，然后平均池化
